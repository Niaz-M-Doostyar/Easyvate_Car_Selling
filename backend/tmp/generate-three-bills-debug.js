...
(async function run() {
  console.log('Starting sequential invoice generation to', outDir);
  for (let i = 0; i < sales.length; i++) {
    const s = sales[i];
    console.log(`Generating (${i+1}/${sales.length}) ${s.saleType} ${s.saleId} ...`);
    try {
      const res = await generateSaleInvoicePdf(s, vehicle, customer, outDir);
      console.log('-> OK:', res.fileName, res.filePath);
    } catch (err) {
      console.error('-> ERROR generating', s.saleId, err && err.stack ? err.stack : err);
    }
  }
  console.log('Done.');
})();
